...
//outfunc
uint16_t jpg_load_buff;
int jpeg_out_func(JDEC* jd, void* bitmap, JRECT* rect) {
	uint32_t vram = VRAM1_BASE;
	//uint32_t buffer_pos = 0;
	uint32_t height = rect->bottom - rect->top + 1;
	uint32_t width = rect->right - rect->left + 1;
	uint32_t x = rect->left;
	uint32_t y = rect->top;
	char* picture_color_info = bitmap;
	//char transparency = 0xFF;
	//uint32_t* test = bitmap;
	
	vram += (y*jd->width + x)*4;
	//buffer_pos += (y*jd->width + x)*4;
	
	for(int pixel_num = 0; pixel_num < height*width; pixel_num++) {
		*(char*)vram = picture_color_info[2];
		*(char*)(vram + 1) = picture_color_info[1];
		*(char*)(vram + 2) = picture_color_info[0];
		*(char*)(vram + 3) = 0xff;
		//mov_in(jpg_load_buff, &picture_color_info[2], (char*)buffer_pos, 1);
		//mov_in(jpg_load_buff, &picture_color_info[1], (char*)buffer_pos + 1, 1);
		//mov_in(jpg_load_buff, &picture_color_info[0], (char*)buffer_pos + 2, 1);
		//mov_in(jpg_load_buff, &transparency, (char*)buffer_pos + 3, 1);
		if (pixel_num%width == width - 1) {
			vram += (jd->width - width)*4;
			//buffer_pos += (jd->width - width)*4;
		}
		vram+=4;
		//buffer_pos+=4;
		picture_color_info+=3;
	}
	return 1;
}

//infunc
size_t jpeg_in_func(JDEC* jd, uint8_t* buf, size_t num) {
	UINT read_byte_num;
	if (buf) {
		f_read((FIL*)jd->device, buf, (UINT)num, &read_byte_num);
		return read_byte_num;
	} else {
		return (f_lseek((FIL*)jd->device,f_tell((FIL*)jd->device)+num)==FR_OK)? num:0;
	}
}

//self make decode function.
JRESULT jpg_decode(char* pic_path) {
	JRESULT result = JDR_OK;
	if (f_open(jpeg_fil, pic_path, FA_READ) != FR_OK) {
		return JDR_INP;
	}
	result = jd_prepare(jpeg_dev, jpeg_in_func, jpg_buffer, jpg_buffer_size, jpeg_fil);
	if (result != JDR_OK) {
		return result;
	}
	result = jd_decomp(jpeg_dev, jpeg_out_func, 0);
	if (result != JDR_OK) {
		return result;
	}
	if (f_close(jpeg_fil) != FR_OK) {
		return JDR_INP;
	}
	return result;
}
